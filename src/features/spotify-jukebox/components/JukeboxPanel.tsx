  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      const payload = event.data as
        ...
        | undefined;
      if (!payload || payload.type !== "soundasta-spotify-auth") return;
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 px-4 py-3 text-sm text-cyan-100">
        Keep asta open on <code className="rounded bg-slate-900/70 px-1">{localhostOrigin}</code>.
