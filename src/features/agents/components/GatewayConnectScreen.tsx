...
export const GatewayConnectScreen = ({
  ...
  const localGatewayCommand = useMemo(
    () => `npx Asta gateway run --bind loopback --port ${localPort} --verbose`,
    [localPort]
  );
  const localGatewayCommandPnpm = useMemo(
    () => `pnpm Asta gateway run --bind loopback --port ${localPort} --verbose`,
    [localPort]
  );
  ...
  const selectedAdapterHint = useMemo(() => {
    switch (selectedAdapterType) {
      case "Asta":
        return "Asta is the provider-rich gateway path. Use this when you want upstream model/provider routing managed by Asta itself.";
      case "hermes":
      ...
      case "asta":
        return "asta runtime preserves asta transcript conventions over the direct runtime seam.";
      case "custom":
      default:
  ...
  const remoteForm = (
    <div className="mt-2.5 flex flex-col gap-3">
      ...
      {showApprovalHint && selectedAdapterType === "Asta" ? (
        <div className="rounded-md border border-border bg-muted/40 px-3 py-3 text-xs text-muted-foreground">
          <p className="leading-snug">
            If the first connection attempt did not work, go to your Asta computer and approve this
            device:
          </p>
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="rounded-md border border-border bg-muted/30 px-3 py-3">
            ...
            <p className="mt-1 text-xs leading-snug text-muted-foreground">
              ...
              <span className="font-mono text-foreground"> asta runtime</span>, or
          ...
          <div className="rounded-md border border-border bg-muted/30 px-3 py-3">
            <p className="text-xs font-medium text-foreground">Opening asta from another machine?</p>
            <p className="mt-1 text-xs leading-snug text-muted-foreground">
              ...
              beyond localhost. Gateway settings are stored on the Studio host, but Asta device approval
            ...
            <div className="ui-input rounded-md px-3 py-3">
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">
                  Use token from <span className="font-mono">~/.Asta/Asta.json</span>.
                </p>
