...
const makeCard = (
  ...
  return {
    ...
    source: input.source ?? "asta_manual",
...
const buildCardFromExplicitEvent = (
  ...
  return makeCard({
    ...
    source:
      explicit.kind === "playbook_triggered" ? "playbook" : "Asta_event",
...
const buildCardFromGatewayTask = (
  ...
  makeCard({
    ...
    status: task.status,
    source: task.source ?? existing?.source ?? "Asta_event",
...
const buildCardFromSharedTaskRecord = (
  ...
  makeCard({
    ...
    description: task.description ?? existing?.description ?? "",
    status: task.status,
    source: task.source ?? existing?.source ?? "asta_manual",
    sourceEventId: task.sourceEventId ?? existing?.sourceEventId ?? null,
    assignedAgentId: task.assignedAgentId ?? existing?.assignedAgentId ?? null,
    createdAt: task.createdAt,
    updatedAt: task.updatedAt,
  ...
  const refreshRemoteTasks = useCallback(async () => {
    ...
    try {
      ...
      setGatewayTasksError(
        error instanceof Error
          ? error.message
          : "Failed to load tasks from Asta.",
      );
    } finally {
      setGatewayTasksLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const snapshot = JSON.stringify(
      stateRef.current.cards
        .filter((card) => !card.isArchived && card.source === "Asta_event")
        .map((card) => ({
          id: card.id,
          title: card.title,
    ...
    for (const card of stateRef.current.cards) {
      if (card.isArchived || card.source !== "Asta_event") continue;
    ...
    (event: EventFrame) => {
      ...
      if (event.event === "agent") {
        ...
        if (!candidate && phase === "start") {
          // Asta started an agent run -- trust that as the classification signal.
