...
const resolveAndValidateLocalMediaPath = (raw: string): { resolved: string; mime: string } => {
  ...
  const allowedRoot = path.join(os.homedir(), ".Asta");
...
const validateRemoteMediaPath = (raw: string): { remotePath: string; mime: string } => {
  ...
  // Remote side enforces ~/.Asta; this guard lets Studio on macOS request
  // /home/ubuntu/.Asta/... without tripping local homedir checks.
  const normalized = trimmed.replaceAll("\\\\", "/");
  const inAsta =
    normalized === "~/.Asta" ||
    normalized.startsWith("~/.Asta/") ||
    normalized.includes("/.Asta/");
  if (!inAsta) {
    throw new Error("Refusing to read remote media outside ~/.Asta");
  }
