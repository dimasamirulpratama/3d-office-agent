...
const CONFIGURED_Asta_PACKAGE_ROOT = process.env.Asta_PACKAGE_ROOT?.trim() ?? "";

const Asta_DIST_INDEX_RELATIVE_PATH = path.join("dist", "index.js");
const Asta_DIST_DIRECTORY_RELATIVE_PATH = "dist";
...
type AstaConfigModule = {
  t: () => AstaConfig;
};

type AstaRunnerModule = {
  a: (params: {
    capability: "audio";
    cfg: AstaConfig;
...
type AstaTranscriptionSdk = {
  loadConfig: AstaConfigModule["t"];
  runCapability: AstaRunnerModule["a"];
  createMediaAttachmentCache: AstaRunnerModule["n"];
  normalizeMediaAttachments: AstaRunnerModule["r"];
  buildProviderRegistry: AstaRunnerModule["t"];
};

export type AstaVoiceTranscriptionResult = {
...
let sdkPromise: Promise<AstaTranscriptionSdk> | null = null;
const nativeImport = new Function(
  "specifier",
  "return import(specifier);",
) as (specifier: string) => Promise<unknown>;

const resolveInstalledAstaPackageRoot = (): string | null => {
  const packageDirName = ["open", "asta"].join("");
  ...
  while (cursor && !visited.has(cursor)) {
    ...
    const indexPath = path.join(candidate, Asta_DIST_INDEX_RELATIVE_PATH);
...
export const buildVoiceTranscriptionErrorMessage = (
  ...
  if (!decision) return "Asta did not return a transcript.";
  ...
  switch (outcome) {
    case "disabled":
      return `Asta audio transcription is disabled.${detail}`.trim();
    case "no-attachment":
      return "Asta did not receive any audio to transcribe.";
    case "scope-deny":
      return `Asta blocked audio transcription for this request.${detail}`.trim();
    case "skipped":
      return `Asta skipped audio transcription.${detail}`.trim();
    default:
      return `Asta did not return a transcript.${detail}`.trim();
  }
};
...
const resolveAstaPackageRoot = (): string => {
  const configuredCandidate = CONFIGURED_Asta_PACKAGE_ROOT;
  if (configuredCandidate) {
    const indexPath = path.join(configuredCandidate, Asta_DIST_INDEX_RELATIVE_PATH);
    if (fs.existsSync(indexPath)) return configuredCandidate;
    throw new Error("Asta_PACKAGE_ROOT does not point to a valid Asta installation.");
  }

  const installedCandidate = resolveInstalledAstaPackageRoot();
  if (installedCandidate) {
    const indexPath = path.join(installedCandidate, Asta_DIST_INDEX_RELATIVE_PATH);
    if (fs.existsSync(indexPath)) return installedCandidate;
  }

  throw new Error(
    "Asta could not be resolved from the current Node runtime. Install the `Asta` package or set Asta_PACKAGE_ROOT.",
  );
};

const loadAstaSdk = async (): Promise<AstaTranscriptionSdk> => {
  if (sdkPromise) return sdkPromise;
  sdkPromise = (async () => {
    const packageRoot = resolveAstaPackageRoot();
    const distDirectory = path.join(packageRoot, Asta_DIST_DIRECTORY_RELATIVE_PATH);
    ...
    if (!loadConfig) {
      throw new Error("The installed Asta runtime does not expose a loadConfig() module.");
    }
    ...
    throw new Error("The installed Asta runtime does not expose the audio transcription runner.");
  })().catch((error) => {
    sdkPromise = null;
    throw error;
  });
...
export const transcribeVoiceWithAsta = async (params: {
  buffer: Buffer;
  fileName?: string | null;
  mimeType?: string | null;
}): Promise<AstaVoiceTranscriptionResult> => {
  const sdk = await loadAstaSdk();
  const cfg = sdk.loadConfig();
  if (cfg.tools?.media?.audio?.enabled === false) {
    throw new Error("Asta audio transcription is disabled.");
  }

  const mimeType = normalizeVoiceMimeType(params.mimeType);
  const fileName = sanitizeVoiceFileName(params.fileName, mimeType);
  const tempDirectory = await fsp.mkdtemp(path.join(os.tmpdir(), "asta-voice-"));
