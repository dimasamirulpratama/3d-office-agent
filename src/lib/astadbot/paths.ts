...
const LEGACY_STATE_DIRNAMES = [".astadbot", ".moltbot"] as const;
const NEW_STATE_DIRNAME = ".Asta";
const CONFIG_FILENAME = "Asta.json";
const LEGACY_CONFIG_FILENAMES = ["astadbot.json", "moltbot.json"] as const;
...
export const resolveConfigPathCandidates = (
  ...
  const explicit =
    env.Asta_CONFIG_PATH?.trim() ||
    env.MOLTBOT_CONFIG_PATH?.trim() ||
    env.astadbot_CONFIG_PATH?.trim();
