...
const STORE_DIR = path.join("asta", "task-manager");
...
const normalizeTaskRecord = (value: unknown): SharedTaskRecord | null => {
  ...
  return {
    ...
    source: isTaskBoardSource(record.source) ? record.source : "asta_manual",
...
export const upsertSharedTask = (
  ...
  const rawSource = (task.source as TaskBoardSource | undefined) ?? existing?.source ?? "asta_manual";
  ...
  const next: SharedTaskRecord = {
    ...
    source: isTaskBoardSource(rawSource) ? rawSource : "asta_manual",
