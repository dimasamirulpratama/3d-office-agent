...
const WORKSPACE_SOURCES = new Set(["Asta-workspace", "agents-skills-personal", "agents-skills-project"]);
...
const resolveGroupId = (skill: SkillStatusEntry): SkillSourceGroupId => {
  ...
  const bundled = skill.bundled || source === "Asta-bundled";
  if (bundled) return "built-in";
  if (WORKSPACE_SOURCES.has(source)) return "workspace";
  if (source === "Asta-managed") return "installed";
  if (source === "Asta-extra") return "extra";
  return "other";
};
...
export const buildSkillMissingDetails = (skill: SkillStatusEntry): string[] => {
  ...
  if (config.length > 0) {
    details.push(`Missing config values (set in Asta.json): ${config.join(", ")}`);
  }
...
export const isBundledBlockedSkill = (skill: SkillStatusEntry): boolean => {
  const source = trimNonEmpty(skill.source) ?? "";
  return (skill.bundled || source === "Asta-bundled") && !skill.eligible;
};
