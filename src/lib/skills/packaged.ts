...
metadata: {"Asta":{"skillKey":"todo-board"}}
---
...
metadata: {"Asta":{"skillKey":"task-manager"}}
---
...
- \`\${Asta_STATE_DIR}/asta/task-manager/tasks.json\` when \`Asta_STATE_DIR\` is set.
- \`~/.Asta/asta/task-manager/tasks.json\` otherwise.
...
// Keep this string synchronized with assets/skills/soundasta/SKILL.md.
const SOUNDasta_SKILL_MD = `---
name: soundasta
description: Control Spotify playback, search music, and return shareable music links.
metadata: {"Asta":{"skillKey":"soundasta"}}
---
...
## Asta Gateway Skill Contract

> This section is for developers implementing the backend skill handler in Asta.
> The asta UI handles authentication via Spotify PKCE OAuth in the browser.
> The gateway skill handles agent-driven requests via the \`soundasta.*\` RPC namespace.

### Authentication model

The user authenticates directly in the browser (PKCE, no secret required).
The access token is stored in browser \`localStorage\` under the key \`soundasta_token\`.
...
soundasta.search({ query: string }): SpotifySearchResult[]

// Get a shareable Spotify link for a query (for Telegram/chat replies).
soundasta.getLink({ query: string }): { url: string; title: string }

// Report current playback state (reads from Spotify API).
soundasta.playerStatus(): PlayerStatus | null

// Request playback of a URI (requires user to be authenticated in browser).
soundasta.play({ uri: string }): { ok: boolean; message?: string }

// Pause / resume / skip.
soundasta.pause(): void
soundasta.resume(): void
soundasta.next(): void
soundasta.previous(): void
\`\`\`
...
3. Agent calls \`soundasta.search\` to find the best match
4. If the request came from a chat channel (Telegram, etc.): call \`soundasta.getLink\` and reply with the link
5. If the request came from the office UI: call \`soundasta.play\` to start playback
