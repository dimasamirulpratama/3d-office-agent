...
> = {
  ...
  "todo-board": {
    ...
    featured: true,
    editorBadge: "asta test",
    hideStats: true,
  },
  "task-manager": {
    category: "Productivity",
    tagline:
      "Turns actionable requests into persistent shared tasks that power the asta Kanban board.",
    capabilities: [
...
const buildFallbackMetadata = (
  ...
  const category =
    skill.bundled || source === "Asta-bundled"
      ? "Built-in"
      : source === "Asta-managed"
        ? "Installed"
  ...
  return {
    ...
    featured: skill.bundled || source === "Asta-managed",
...
export const buildSkillMarketplaceEntry = (
  ...
  if (packagedSkill && !skill.baseDir.trim()) {
    missingDetails.unshift(
      "Install this packaged asta skill to make it available on the gateway.",
    );
  }
  return {
    skill,
...
export const buildSkillMarketplaceCollections = (
  ...
  const asta = entries.filter((entry) =>
    getPackagedSkillBySkillKey(entry.skill.skillKey),
  );
  if (asta.length > 0) {
    collections.push({ id: "asta", label: "asta", entries: asta });
  }
