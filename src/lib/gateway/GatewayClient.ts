...
} from "./openasta/GatewayBrowserClient";
import type {
...
const Asta_CONTROL_UI_CLIENT_ID = "Asta-control-ui";
const Asta_WEBCHAT_UI_CLIENT_ID = "webchat-ui";

const isAutoManagedAdapter = (adapterType: StudioGatewayAdapterType) =>
  adapterType === "Asta" || adapterType === "hermes" || adapterType === "demo";
...
export const resolveGatewayClientName = (
  ...
  if (adapterType !== "Asta") {
    return Asta_CONTROL_UI_CLIENT_ID;
  }
  return isLocalGatewayUrl(gatewayUrl)
    ? Asta_CONTROL_UI_CLIENT_ID
    : Asta_WEBCHAT_UI_CLIENT_ID;
};
...
const normalizeGatewayProfilesPublic = (
  ...
  for (const adapterType of ["Asta", "hermes", "demo", "local", "asta", "custom"] as const) {
...
const doctorFixHint =
  "Run `npx Asta doctor --fix` on the gateway host (or `pnpm Asta doctor --fix` in a source checkout).";

const protocolMismatchHint =
  "This gateway looks too old for asta's protocol v3. Upgrade Asta, use the Hermes adapter, or run `npm run demo-gateway` for a no-framework office demo.";

const tailscaleGatewayHint =
  "If this is a remote Asta/Tailscale gateway, confirm the Studio host can reach the `wss://...` address and approve the first device pairing on the gateway host with `Asta devices approve --latest`.";

const pairingRequiredHint =
  "This gateway is asking for first-time device approval. Run `Asta devices approve --latest` on the gateway host, then restart asta and reconnect from this browser.";

const requiresDeviceIdentityHint =
  "This gateway rejected the client as a control UI without device identity. For remote Asta/Tailscale connections, update to the latest asta build and approve the device pairing on the gateway host.";
...
export const useGatewayConnection = (
  ...
  const [selectedAdapterType, setSelectedAdapterTypeState] =
    useState<StudioGatewayAdapterType>("Asta");
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        try {
          await client.connect({
            ...
            disableDeviceAuth: selectedAdapterType !== "Asta",
          });
          lastError = null;
          break;
        } catch (err) {
  ...
  const disconnect = useCallback(() => {
    ...
    // (e.g. switching from Asta → local sets selectedAdapterType before disconnect
  ...
  const shouldPromptForConnect =
    ...
    (selectedAdapterType === "custom" ||
      ...
      !(gatewayUrl ?? "").trim() ||
      (selectedAdapterType === "Asta" && !(token ?? "").trim()) ||
