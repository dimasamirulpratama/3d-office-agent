...
import { normalizeGatewayEvent } from "@/lib/runtime/openasta/normalizeGatewayEvent";
import type { RuntimeCapability, RuntimeEvent, RuntimeProvider } from "@/lib/runtime/types";

const Asta_RUNTIME_CAPABILITIES: ReadonlySet<RuntimeCapability> = new Set([
  "agents",
  "sessions",
  "chat",
  "agent-messages",
...
export class AstaRuntimeProvider implements RuntimeProvider {
  readonly id = "Asta" as const;
  readonly label = "Asta";
  readonly metadata = {
    id: this.id,
    label: this.label,
  ...
  readonly capabilities = Asta_RUNTIME_CAPABILITIES;
