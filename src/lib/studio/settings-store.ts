...
import { resolveStateDir } from "@/lib/astadbot/paths";
import {
...
const SETTINGS_DIRNAME = "asta";
const SETTINGS_FILENAME = "settings.json";
const Asta_CONFIG_FILENAME = "Asta.json";
...
const readAstaGatewayDefaults = (): StudioGatewaySettings | null => {
  try {
    const configPath = path.join(resolveStateDir(), Asta_CONFIG_FILENAME);
...
const buildEnvGatewayDefaults = (): StudioGatewaySettings | null => {
  const envUrl = process.env.ASTA_GATEWAY_URL?.trim();
  const envToken = process.env.ASTA_GATEWAY_TOKEN?.trim() ?? "";
  const envAdapterType =
    normalizeAdapterType(process.env.ASTA_GATEWAY_ADAPTER_TYPE) ?? "hermes";
...
export const loadLocalGatewayDefaults = (): StudioGatewaySettings | null => {
  const fromFile = readAstaGatewayDefaults();
