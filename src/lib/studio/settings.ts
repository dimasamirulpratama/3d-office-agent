...
const DEFAULT_Asta_GATEWAY_URL = "ws://localhost:18789";
const DEFAULT_LOCAL_ADAPTER_GATEWAY_URL = "ws://localhost:18789";
const DEFAULT_LOCAL_RUNTIME_URL = "http://localhost:7770";
const DEFAULT_ASTA_RUNTIME_URL = "http://localhost:3000/api/runtime/custom";
...
const DEFAULT_OFFICE_TITLE = "Dimas Headquarters";
...
const normalizeRemoteOfficeSourceKind = (
  ...
  if (kind === "presence_endpoint" || kind === "Asta_gateway") {
    return kind;
  }
  return fallback;
};
...
const normalizeGatewayAdapterType = (
  value: unknown,
  fallback: StudioGatewayAdapterType = "Asta"
...
export const resolveDefaultStudioGatewayProfile = (
  ...
  switch (adapterType) {
    case "asta":
      return { url: DEFAULT_ASTA_RUNTIME_URL, token: "" };
    case "local":
    ...
    case "Asta":
    default:
      return { url: DEFAULT_Asta_GATEWAY_URL, token: "" };
  }
};
