...
the main runtime stabilization branch for asta.
...
- extends the runtime seam beyond Asta-only assumptions
...
This branch no longer only has a single Asta-shaped runtime path.
It now includes:
...
- reduced Asta-specific assumptions leaking into Hermes/demo/custom paths
...
- provider selection is no longer effectively Asta-only
