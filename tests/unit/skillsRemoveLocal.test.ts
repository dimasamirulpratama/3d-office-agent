...
const mkTmpDir = () => fs.mkdtempSync(path.join(os.tmpdir(), "asta-skill-remove-"));

describe("skills remove local", () => {
  const originalStateDir = process.env.Asta_STATE_DIR;

  afterEach(() => {
    if (originalStateDir === undefined) delete process.env.Asta_STATE_DIR;
    else process.env.Asta_STATE_DIR = originalStateDir;
  });

  it("removes a workspace skill directory", () => {
    process.env.Asta_STATE_DIR = mkTmpDir();
