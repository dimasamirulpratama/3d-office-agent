...
describe("studio settings normalization", () => {
  ...
  it("creates default per-floor runtime state", () => {
    ...
    expect(normalized.officeFloors["Asta-ground"]).toEqual(
      expect.objectContaining({
        floorId: "Asta-ground",
        provider: "Asta",
        runtimeProfileId: "Asta-default",
        gatewayUrl: null,
        status: "disconnected",
      }),
    );
    ...
    const resolved = resolveStudioGatewayProfiles({
      gateway: normalizeStudioSettings({
        gateway: {
          ...
          adapterType: "hermes",
          profiles: {
            Asta: { url: "ws://localhost:18789", token: "open-token" },
  ...
  it("resolves adapter-specific defaults for dormant profiles", () => {
    ...
    expect(resolveDefaultStudioGatewayProfile("asta", null)).toEqual({
