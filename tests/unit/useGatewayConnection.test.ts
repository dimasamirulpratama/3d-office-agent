...
const setupAndImportHook = async (gatewayUrl: string | null) => {
  ...
  vi.doMock("../../src/lib/gateway/Asta/GatewayBrowserClient", () => {
    class GatewayBrowserClient {
      ...
      start() {
        this.connected = true;
        this.opts.onHello?.({ type: "hello-ok", protocol: 1, adapterType: "Asta" });
      }
  ...
  return {
    useGatewayConnection: mod.useGatewayConnection as (settingsCoordinator: {
      ...
      gatewayUrl: string;
      token: string;
      selectedAdapterType: "Asta" | "hermes" | "demo" | "custom";
      detectedAdapterType: "Asta" | "hermes" | "demo" | "custom" | null;
      activeAdapterType: "Asta" | "hermes" | "demo" | "custom";
      localGatewayDefaults: {
        url: string;
        token: string;
        adapterType: "Asta" | "hermes" | "demo" | "custom";
      } | null;
      shouldPromptForConnect: boolean;
      useLocalGatewayDefaults: () => void;
      setSelectedAdapterType: (value: "Asta" | "hermes" | "demo" | "custom") => void;
...
describe("useGatewayConnection", () => {
  ...
  it("connects_via_studio_proxy_ws_and_does_not_pass_token", async () => {
    ...
    expect(captured.clientName).toBe("Asta-control-ui");
  });

  it("uses_webchat_identity_for_remote_Asta_connections", async () => {
    const { useGatewayConnection, captured } = await setupAndImportHook(null);
    const coordinator = {
      loadSettings: async () => null,
      loadSettingsEnvelope: async () => ({
  ...
  it("keeps_control_ui_identity_for_local_Asta_connections", async () => {
    const { useGatewayConnection, captured } = await setupAndImportHook(null);
    const coordinator = {
      loadSettings: async () => null,
      loadSettingsEnvelope: async () => ({
  ...
  it("uses_a_small_initial_auto_connect_delay_for_hermes_and_demo_only", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(mod.resolveInitialGatewayAutoConnectDelayMs("Asta")).toBe(0);
  ...
  it("retries_only_the_first_connect_for_hermes_and_demo", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(mod.resolveInitialGatewayConnectAttemptCount("Asta", false)).toBe(1);
    ...
    expect(mod.resolveInitialGatewayConnectAttemptCount("Asta", true)).toBe(1);
  });

  it("uses_webchat_client_id_only_for_remote_Asta", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(mod.resolveGatewayClientName("Asta", "wss://pi5.myth-coho.ts.net")).toBe(
      "webchat-ui"
    );
    expect(mod.resolveGatewayClientName("Asta", "ws://localhost:18789")).toBe(
      "Asta-control-ui"
    );
  ...
  it("auto_applies_runtime_local_defaults_when_no_saved_gateway_and_build_time_empty", async () => {
    ...
    // ASTA_GATEWAY_URL is set on the server so localGatewayDefaults
  ...
  it("loads_and_persists_selected_adapter_type", async () => {
    ...
    expect(firstPatch.gateway?.profiles?.Asta?.token).toBe("");
