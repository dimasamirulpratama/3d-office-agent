...
describe("skills install gateway", () => {
  it("creates a temporary installer agent and installs a workspace skill", async () => {
    ...
    expect(result).toEqual({
      installed: true,
      installedPath: "/home/Asta/workspace-demo/skills/todo-board",
      source: "Asta-workspace",
      skillKey: "todo-board",
    });
    ...
    await expect(
      installPackagedSkillViaGatewayAgent({
        ...
        request: {
          packageId: "todo-board",
          source: "Asta-workspace",
          workspaceDir: "/home/pi/.Asta/workspace",
          managedSkillsDir: "/home/pi/.Asta/skills",
          agentId: "soundasta",
          agentName: "soundasta",
        },
      })
  ...
  it("repairs the workspace from agent file provenance before creating the installer agent", async () => {
    const call = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      if (method === "agents.files.get") {
        ...
        return {
          workspace: "/home/pi/.Asta/workspace",
          file: {
            missing: false,
            content: "# IDENTITY",
            path: "/home/pi/.Asta/workspace-main/IDENTITY.md",
          },
        };
      }
      if (method === "agents.create") {
    ...
    expect(result.installedPath).toBe("/home/pi/.Asta/workspace-main/skills/todo-board");
    expect(call).toHaveBeenCalledWith("agents.create", {
      name: expect.stringContaining("Skill Installer"),
      workspace: "/home/pi/.Asta/workspace-main",
    });
  });
});
