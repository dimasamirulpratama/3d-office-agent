...
describe("skills presentation helpers", () => {
  it("groups skills by source with stable ordering", () => {
    const groups = groupSkillsBySource([
      ...
      createSkill({ name: "installed", source: "Asta-managed" }),
      createSkill({ name: "workspace", source: "Asta-workspace" }),
      createSkill({ name: "bundled", source: "Asta-bundled", bundled: true }),
      createSkill({ name: "extra", source: "Asta-extra" }),
    ]);
  ...
  it("builds explicit missing detail lines", () => {
    ...
    expect(details).toEqual([
      ...
      "Missing config values (set in Asta.json): browser.enabled",
      "Requires OS: Linux",
    ]);
  });
  ...
  it("marks only gateway-managed and workspace skill sources as removable", () => {
    expect(canRemoveSkill(createSkill({ source: "Asta-managed" }))).toBe(true);
    expect(canRemoveSkill(createSkill({ source: "Asta-workspace" }))).toBe(true);
    ...
    expect(canRemoveSkill(createSkill({ source: "Asta-bundled", bundled: true }))).toBe(
      false
    );
    expect(canRemoveSkill(createSkill({ source: "Asta-extra" }))).toBe(false);
  });
