...
describe("server studio upstream gateway settings", () => {
  const priorStateDir = process.env.Asta_STATE_DIR;
  let tempDir: string | null = null;

  afterEach(() => {
    process.env.Asta_STATE_DIR = priorStateDir;
    if (tempDir) {
  ...
  it("falls back to Asta.json token/port when studio settings are missing", async () => {
    tempDir = makeTempDir("studio-upstream-Asta-defaults");
    process.env.Asta_STATE_DIR = tempDir;

    fs.writeFileSync(
      path.join(tempDir, "Asta.json"),
  ...
  it("keeps a configured url and fills token from Asta.json when missing", async () => {
    tempDir = makeTempDir("studio-upstream-url-keep");
    process.env.Asta_STATE_DIR = tempDir;

    fs.mkdirSync(path.join(tempDir, "asta"), { recursive: true });
    fs.writeFileSync(
      path.join(tempDir, "asta", "settings.json"),
