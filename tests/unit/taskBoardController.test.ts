...
describe("task board controller helpers", () => {
  it("parses explicit Asta task events", () => {
  ...
  it("treats plain inbound user asks as live session tasks", () => {
    const card = deriveLiveSessionTaskCard(
      {
        ...
        payload: {
          ...
          message: {
            role: "user",
            content: [{ type: "text", text: "Can you check the latest news on Asta?" }],
          },
        },
      },
      [makeAgent()],
    ...
    expect(card).toEqual(
      expect.objectContaining({
        ...
        title: "Can you check the latest news on Asta?",
        assignedAgentId: "agent-1",
        channel: "telegram",
      ...
      makeAgent({
        ...
        transcriptEntries: [
          ...
          },
          {
            ...
            role: "user",
            kind: "user",
            text: "Can you check the latest news on Asta?",
            sessionKey: "agent:agent-1:main",
            runId: null,
            source: "history",
