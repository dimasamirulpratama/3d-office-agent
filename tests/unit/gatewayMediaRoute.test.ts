...
const writeStudioSettings = (stateDir: string, gatewayUrl: string) => {
  const settingsDir = path.join(stateDir, "asta");
...
describe("/api/gateway/media route", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.Asta_GATEWAY_SSH_TARGET;
    delete process.env.Asta_GATEWAY_SSH_USER;
    delete process.env.Asta_STATE_DIR;
  ...
  it("returns binary image data when reading remote media over ssh", async () => {
    ...
    process.env.Asta_GATEWAY_SSH_TARGET = "me@host.test";
    ...
    const remotePath = "/home/ubuntu/.Asta/images/pic.png";
  ...
  it("rejects symlinked local media paths", async () => {
    ...
    const allowedRoot = path.join(realHome, ".Asta");
