...
describe("useRuntimeConnection", () => {
  ...
  it("selects the asta runtime provider from the active adapter type", async () => {
    vi.doMock("@/lib/gateway/GatewayClient", () => ({
      useGatewayConnection: () => ({
        client: {},
        status: "connected",
    ...
    expect(screen.getByTestId("providerId")).toHaveTextContent("asta");
    expect(screen.getByTestId("providerLabel")).toHaveTextContent("asta Runtime");
  });
});
