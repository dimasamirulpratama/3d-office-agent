...
import {
  ...
  shouldRunAstaChecks,
  summarizeChecks,
} from "../../scripts/lib/astadoctor-core.mjs";

describe("astaoctor core", () => {
  it("resolves selected runtime from settings profiles", () => {
    ...
    >;
    expect(profiles.Asta?.url).toBe("ws://localhost:18789");
  });
  ...
  it("supports local and asta runtime defaults", () => {
    expect(
      resolveRuntimeContext({
    ...
    expect(
      resolveRuntimeContext({
        settings: { gateway: { adapterType: "asta" } },
        upstreamGateway: { url: "", token: "", adapterType: "asta" },
        env: process.env,
      }).gatewayUrl,
  ...
  it("warns about remote Asta tunnel setups without a token", () => {
    expect(
      buildAstaWarnings({
  ...
  it("warns when multiple runtime profiles share the same endpoint", () => {
    expect(
      buildProfileWarnings({
        runtimeContext: {
          profiles: {
            Asta: { url: "ws://localhost:18789", token: "a" },
  ...
  it("builds remediation actions from tunnel and pairing style failures", () => {
    expect(
      ...
      expect.arrayContaining([
        expect.stringContaining("Asta devices list"),
  ...
  it("enables provider-specific checks based on runtime and local state", () => {
    ...
    expect(
      shouldRunCustomChecks({
        runtimeContext: { adapterType: "asta" },
      }),
    ).toBe(true);
  });

  it("treats local and asta as custom-runtime adapters", () => {
    expect(isCustomRuntimeAdapter("custom")).toBe(true);
    expect(isCustomRuntimeAdapter("local")).toBe(true);
    expect(isCustomRuntimeAdapter("asta")).toBe(true);
    expect(isCustomRuntimeAdapter("Asta")).toBe(false);
  });
  ...
  it("builds a structured json report", () => {
    const report = buildDoctorJsonReport({
      ...
      paths: {
        stateDir: "C:/tmp/.Asta",
        settingsPath: "C:/tmp/.Asta/asta/settings.json",
      },
      checks: [
        {
          status: DOCTOR_STATUSES.warn,
  ...
  it("formats a grouped terminal report with configured profiles", () => {
    const report = formatDoctorReport({
      ...
      runtimeContext: {
        ...
        profiles: {
          hermes: { url: "ws://localhost:18789", token: "" },
          Asta: { url: "ws://localhost:28789", token: "secret" },
        },
      },
      paths: {
        stateDir: "C:/tmp/.Asta",
    ...
    expect(report).toContain("astaoctor");
...
describe("parseDoctorArgs", () => {
  ...
  it("combines flags", () => {
    expect(parseDoctorArgs(["--json", "--profile", "Asta"])).toEqual({
      json: true,
      allProfiles: false,
      profile: "Asta",
    });
...
describe("adapterInScope scoping semantics", () => {
  // Mirror the adapterInScope helper used in astaoctor.mjs so the logic can
  ...
  it("default (no flags): delegates to defaultBehavior", () => {
    ...
    expect(inScope("Asta", true)).toBe(true);
    expect(inScope("Asta", false)).toBe(false);
  ...
  it("--profile hermes: only hermes is in scope", () => {
    ...
    expect(inScope("Asta", true)).toBe(false); // Asta would default to true but is suppressed
    expect(inScope("demo", true)).toBe(false);
    expect(inScope("custom", false, ["local", "asta"])).toBe(false);
  });

  it("--profile Asta: only Asta is in scope", () => {
    const inScope = makeAdapterInScope({
      allProfiles: false,
      profile: "Asta",
    });
    expect(inScope("Asta", false)).toBe(true);
  ...
  it("--profile local: custom-runtime checks stay in scope", () => {
    ...
    expect(inScope("custom", false, ["local", "asta"])).toBe(true);
    expect(inScope("Asta", true)).toBe(false);
  });

  it("--profile asta: custom-runtime checks stay in scope", () => {
    const inScope = makeAdapterInScope({
      allProfiles: false,
      profile: "asta",
    });
