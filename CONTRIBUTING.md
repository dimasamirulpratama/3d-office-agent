...
Thanks for helping improve asta.

Please use GitHub Issues for bugs, feature requests, and questions about planned work.

## Before you start
- Install Asta and confirm the gateway runs locally.
- This repo is UI-only and reads config from `~/.Asta` with legacy fallback to `~/.moltbot` or `~/.astadbot`.
