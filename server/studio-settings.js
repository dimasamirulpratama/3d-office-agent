...
const LEGACY_STATE_DIRNAMES = [".astadbot", ".moltbot"];
const NEW_STATE_DIRNAME = ".Asta";
...
const resolveStateDir = (env = process.env) => {
  const override =
    env.Asta_STATE_DIR?.trim() ||
    env.MOLTBOT_STATE_DIR?.trim() ||
    env.astadbot_STATE_DIR?.trim();
...
const resolveStudioSettingsPath = (env = process.env) => {
  return path.join(resolveStateDir(env), "asta", "settings.json");
};
...
const readAstaGatewayDefaults = (env = process.env) => {
  try {
    const stateDir = resolveStateDir(env);
    const configPath = path.join(stateDir, Asta_CONFIG_FILENAME);
    ...
    if (!url) return null;
    return { url, token, adapterType: "Asta" };
  } catch {
    return null;
  }
};
...
const loadUpstreamGatewaySettings = (env = process.env) => {
  ...
  const adapterType =
    typeof gateway?.adapterType === "string" && gateway.adapterType.trim()
      ...
      : "Asta";
  if (!token && adapterType === "Asta") {
    const defaults = readAstaGatewayDefaults(env);
    if (defaults) {
      return {
