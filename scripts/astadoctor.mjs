...
const detectAstaVersion = () => {
  try {
    return execFileSync("Asta", ["--version"], {
      encoding: "utf8",
...
async function main() {
  ...
  checks.push(
    runtimeContext.gatewayUrl
      ...
      : checkFail(
          ...
          ["Set the gateway URL in asta connect/settings before retrying."],
        ),
  );
  ...
  checks.push(
    runtimeContext.tokenConfigured
      ...
      : checkWarn(
          ...
          [
            "If this backend requires token auth, set the upstream token in asta settings or Asta.json.",
          ],
        ),
  );
  ...
  for (const warning of buildProfileWarnings({ runtimeContext })) {
    checks.push(
      checkWarn("Runtime profiles", "Profile collision", warning, [
        "Assign distinct local ports or URLs if you want Asta, Hermes, and demo running simultaneously instead of swapping one backend onto the same endpoint.",
      ]),
    );
  }
    ...
    checks.push(
      requestedProfile
        ...
        : checkFail(
            ...
            [
              "Run `node scripts/astaoctor.mjs --all-profiles` to see the configured profile list.",
            ],
          ),
    );
  ...
  if (adapterInScope("Asta", runtimeContext.adapterType === "Asta")) {
    for (const warning of buildAstaWarnings({
      gatewayUrl: runtimeContext.gatewayUrl,
      tokenConfigured: runtimeContext.tokenConfigured,
    })) {
      checks.push(
        checkWarn("Asta", "Asta hints", warning, [
          "If the browser/device is not yet approved, check `Asta devices list` and approve the pending device before retrying the remote connection.",
        ]),
      );
    }
  }
  ...
  const AstaConfigPath = path.join(stateDir, "Asta.json");
  const AstaConfigExists = fs.existsSync(AstaConfigPath);
  if (shouldRunAstaChecks({ runtimeContext, AstaConfigExists })) {
    checks.push(
      AstaConfigExists
        ? checkPass(
            "Asta",
            "Asta config",
            `Found ${AstaConfigPath}.`,
          )
        : checkWarn(
            "Asta",
            "Asta config",
            `No Asta.json found at ${AstaConfigPath}.`,
            [
              "If you expect a local Asta default, verify Asta_STATE_DIR or create Asta.json.",
            ],
          ),
    );

    const version = detectAstaVersion();
    const versionLooksValid = /^Asta\s+/i.test(version);
    checks.push(
      versionLooksValid
        ? checkPass("Asta", "Asta version", version)
        : checkWarn("Asta", "Asta version", version, [
            "Install Asta or ensure it is available on PATH if this machine should run it directly.",
          ]),
    );
  }
