#!/usr/bin/env bash
# astad3d-start — Start all Astad3D services, auto-resolving port conflicts.
#
# Setup (once):
#   echo 'alias astad3d="/absolute/path/to/asta/scripts/astad3d-start.sh"' >> ~/.zshrc
#   source ~/.zshrc
#
# Then just run:  astad3d

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
ASTAD3D_DIR="$(cd -- "$SCRIPT_DIR/.." >/dev/null 2>&1 && pwd)"
LOG_DIR="/tmp/astad3d-logs"
mkdir -p "$LOG_DIR"

GREEN='\033[0;32m'; YELLOW='\033[1;33m'; BLUE='\033[0;34m'; NC='\033[0m'
log()  { echo -e "${GREEN}[astad3d]${NC} $*"; }
warn() { echo -e "${YELLOW}[astad3d]${NC} $*"; }
info() { echo -e "${BLUE}[astad3d]${NC} $*"; }
...
if ! port_free $APP_PORT; then
  if port_owned_by $APP_PORT "node.*next|next-server|server/index\.js"; then
    warn "Astad3D dev server already running on :$APP_PORT — reusing."
  else
    APP_PORT=$(find_free_port $((APP_PORT + 1)))
    warn "Port 3000 taken by another process → using :$APP_PORT for Astad3D."
    log "Starting Astad3D dev server on :$APP_PORT..."
    cd "$ASTAD3D_DIR"
    nohup env PORT="$APP_PORT" NEXT_PUBLIC_GATEWAY_URL="$GATEWAY_WS_URL" \
      npm run dev > "$LOG_DIR/astad3d-dev.log" 2>&1 &
  fi
else
  log "Starting Astad3D dev server on :$APP_PORT..."
  cd "$ASTAD3D_DIR"
...
log "Waiting for Astad3D to be ready at :$APP_PORT..."
ready=0
...
if [ "$ready" -eq 0 ]; then
  warn "Timed out waiting for :$APP_PORT — check $LOG_DIR/astad3d-dev.log"
fi
...
echo ""
log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log " Astad3D      →  http://localhost:$APP_PORT"
