...
const tryReadGatewayTokenFromAstaCli = () => {
  try {
    const raw = execFileSync("Asta", ["config", "get", "gateway.auth.token"], {
      encoding: "utf8",
...
async function main() {
  ...
  try {
    ...
    const tokenDefault = tryReadGatewayTokenFromAstaCli();
    const tokenPrompt = tokenDefault
      ? "Upstream Gateway Token [detected from Asta]: "
      : "Upstream Gateway Token: ";
    const tokenAnswer = await rl.question(tokenPrompt);
    const token = (tokenAnswer || tokenDefault || "").trim();
    if (!token) {
      throw new Error(
        "Gateway token is required. Provide it, or install/Asta so it can be auto-detected."
      );
    }
