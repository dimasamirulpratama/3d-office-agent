...
export const normalizeAdapterType = (value, fallback = "Asta") => {
...
export const buildAstaWarnings = ({
  gatewayUrl,
  ...
  if (!tokenConfigured) {
    warnings.push(
      "Remote Asta profile has no gateway token configured. Remote/browser clients often fail with pairing or approval-style errors until the device or token path is approved.",
    );
  }

  if (isTunnelBackedHostname(hostname)) {
    warnings.push(
      "Remote Asta host looks tunnel-backed. If you hit 1008/1011/1012-style failures, verify direct local or LAN access first, then check pairing/device approval and reverse-proxy websocket handling.",
    );
  }
...
export const buildGatewayFailureActions = ({
  ...
  if (normalized.includes("econnrefused") || normalized.includes("timed out")) {
    actions.push(
      "Verify the backend is actually listening on the configured host and port before retrying from asta.",
    );
  }

  if (normalized.includes("1011")) {
    actions.push(
      "If this is Asta behind a reverse proxy or tunnel, verify websocket upgrade handling and compare direct local/LAN behavior before assuming the runtime itself is broken.",
    );
  }
  ...
  if (
    ...
    actions.push(
      "For Asta, check pending device/browser approval with `Asta devices list` and approve the request before retrying the remote browser session.",
    );
  }
  ...
  if (isTailscale) {
    actions.push(
      "For Tailnet-hosted Asta, test the same gateway directly on local/LAN first, then compare against the Tailnet URL so pairing/proxy issues do not get conflated.",
    );
  }
...
export const shouldRunAstaChecks = ({
  runtimeContext,
  AstaConfigExists = false,
}) => runtimeContext.adapterType === "Asta" || AstaConfigExists;
...
export const formatDoctorReport = ({
  ...
  const lines = [];
  lines.push("==================================================");
  lines.push(`astaoctor ${formatStatusBadge(summary)}`);
  lines.push("==================================================");
  lines.push("");
