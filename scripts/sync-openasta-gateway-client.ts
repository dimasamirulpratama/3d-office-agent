...
const requestedSourcePath =
  process.argv[2]?.trim() ||
  process.env.Asta_GATEWAY_CLIENT_SOURCE?.trim() ||
  process.env.Asta_UI_PATH?.trim() ||
  "";
...
if (!sourcePath) {
  console.error(
    "Missing upstream gateway client source path. Provide it as `npm run sync:gateway-client -- /path/to/gateway.ts` or set Asta_GATEWAY_CLIENT_SOURCE."
  );
  process.exit(1);
}
