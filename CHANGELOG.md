...
This release converges the runtime-profiles, office-systems, astaoctor, and selective `vera_lane` work into one branch. Backends become named profiles, the office becomes a multi-floor building, remote offices gain server-backed messaging and handoffs, and a new diagnostics CLI makes setup and troubleshooting first-class.

### Added

- Named runtime profiles for `Asta`, `hermes`, `demo`, `local`, `asta`, and `custom` backends, each storing its own URL and token in Studio settings instead of a single global pair (`docs/runtime-profiles.md`, `src/lib/runtime/*`).
- Multi-floor office runtime model with one runtime binding per floor and persistent per-floor state, including `lobby`, `Asta-ground`, `hermes-first`, `local-runtime`, `asta-runtime`, `custom-second`, `training`, `traders-floor`, and `campus` (`src/lib/office/floors.ts`, `docs/office_sys/multi-floor-runtime-architecture.md`).
...
- `astaoctor` diagnostics CLI with profile-scoped and `--all-profiles` runs, Asta / Hermes / demo / custom-runtime probes, gateway failure classification, JSON output, and a `npm run doctor` script (`scripts/astaoctor.mjs`, `scripts/lib/astaoctor-core.mjs`, `package.json`).
- New product and architecture docs covering runtime profiles, multi-floor architecture, the astaoctor spec, runtime profile architecture, the refreshed roadmap, multi-agent beta, and the bulletin-board, desk-progression, hierarchy-and-teams, meeting-room-workflow, QA-department, and whiteboard specs (`docs/`).

### Changed

- Runtime provider selection is now profile-aware across `Asta`, `hermes`, `demo`, `local`, `asta`, and `custom` instead of collapsing into one generic path (`src/lib/runtime/createRuntimeProvider.ts`, `src/lib/runtime/{Asta,hermes,demo,custom}/provider.ts`).
...
- Resolved a UTF-8 / Turbopack parsing issue and cleaned up Turbopack root and optional `Asta` resolution warnings (`next.config.ts`).
...
- Added unit coverage for `astaoctor`, office floors, floor roster, runtime connection, gateway connection, office floor runtime persistence, agent fleet hydration derivation, and studio settings coordinator behavior (`tests/unit/`).
...
- Expanded `README.md` to describe `Local` and `asta` runtime modes and persistent backend profile configuration, including the additional `local` and `asta` values for `ASTA_GATEWAY_ADAPTER_TYPE`.
...
This release expands asta from a single-office viewer into a more complete AI workplace, with guided setup, richer agent operations, remote office support, and stronger security hardening.
...
- New packaged skills marketplace with trigger-driven office routing, including starter skills like Todo Board and SOUNDASTA.
...
- Added an Agent Bus integration guide for visualizing AI sessions in asta.
...
- A `DEBUG` environment toggle for controlling the Asta event console in the office UI.
...
- Defaulted the Asta event console to a collapsed state and made it optional from environment configuration.
...
- Initial public asta project documentation, including `README.md`, `VISION.md`, and `ARCHITECTURE.md`.
- A gateway-first web UI for connecting to Asta agents, monitoring runtime activity, and managing agent workflows.
