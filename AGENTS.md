...
This repo is a frontend for Asta. Keep any Asta runtime checkout separate from this repository.

Do not modify the Asta source code. When the user asks for changes, they are asking for changes to this app. Your solutions should be applied to this app but to understand the full context of implementing your solution, you will need to search through Asta's source code.
...
asta is a Next.js 16 frontend (TypeScript, React 19, Three.js, Phaser) for Asta. It runs a custom Node.js server (`server/index.js`) that bundles a same-origin WebSocket proxy to the upstream Asta Gateway. No database or Docker is required. The only hard system dependency is Node.js 20+ with npm 10+.

### Running the app

- `npm run dev` starts the dev server on port 3000 via the custom server (`node server/index.js --dev`).
- The app requires a running Asta Gateway to show agent data. Without one, the UI loads but shows the gateway connection form. This is expected and not an error.
...
- `npm run build` — Next.js production build. Expect a non-blocking warning about `Can't resolve 'Asta'`; the `Asta` npm package is resolved optionally at runtime and is not bundled.

### Gotchas

- The `Asta` npm package is not a dependency of this repo. The build warning about it is harmless.
