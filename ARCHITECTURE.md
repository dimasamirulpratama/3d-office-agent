...
## Overview
asta is a gateway-first Next.js application for visualizing and operating AI agents powered by Asta using Three.JS framework.

It is the UI and proxy layer, not the Asta runtime itself. Asta remains the system of record for agents, sessions, and execution, while asta provides:
...
- a Studio-side settings and proxy layer that connects the browser to an upstream Asta gateway.

## Goals
- Keep Asta as the source of truth for runtime state.
...
- Replacing Asta as the execution engine.
- Moving gateway-owned agent state into local frontend storage.

## System Model
asta is split into four main parts:
...
3. Studio WebSocket proxy.
   A custom Node server terminates browser WebSocket connections at `/api/gateway/ws` and forwards them to the upstream Asta gateway.
4. Asta gateway.
   The gateway owns agent records, sessions, config, approvals, and runtime events.

## Core Boundaries
### 1. Gateway-owned state
Agent records, sessions, approvals, runtime streams, and agent files belong to Asta.

asta may read and mutate that state through gateway APIs, but it should not create a competing local source of truth.
...
These settings live under the local Asta state directory and are accessed through server routes, not directly from the browser.
...
## Design Principles
- Gateway first.
  If data belongs to the runtime, it should live in Asta, not in a local frontend file.
...
- Agent configuration and files are managed through gateway APIs.
  This avoids drift between asta and the upstream runtime.
...
- Do not write gateway-owned agent config directly to local Asta config files.
