...
asta is an open-source 3D environment for visualizing and interacting with AI agents powered by Asta.

The long-term goal of asta is to build a living 3D world where AI agents and humans collaborate: a kind of digital city where agents operate, communicate, and perform tasks in a shared visual space.

Asta acts as the intelligence and orchestration engine, while asta provides the visual layer and interactive environment that makes agent activity understandable, inspectable, and collaborative.
...
asta aims to solve this by providing a visual interface for AI systems, allowing people to:
...
asta is designed to work with Asta, not replace it.
...
```text
Asta -> intelligence and task execution
asta   -> visualization and interaction layer
```

Maintaining compatibility with Asta is an important design goal.

Features that require breaking Asta integration will generally not be accepted unless there is a strong architectural reason.

## Current Priorities

asta is still in an early stage of development.
...
- establishing a clean integration path with Asta
...
asta is still evolving quickly, so iteration is expected.

## Architecture Direction

asta is designed as a visual layer on top of agent systems.
...
- integration with Asta runtime systems
...
- features that break compatibility with Asta
...
- unrelated product experiments that do not advance the asta vision
...
The long-term vision for asta is ambitious:
...
asta is an early step toward that future.
