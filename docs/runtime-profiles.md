...
asta now treats runtime backends as named saved profiles instead of one global URL/token pair.
...
The normal Asta gateway flow over Studio's WebSocket bridge.

This is the provider-rich path. Asta already knows how to sit in front of many upstream model providers, so asta should treat it as a first-class gateway adapter rather than flattening it into `custom`.
...
A asta-shaped HTTP runtime profile for stacks that want to keep asta transcript and chat conventions while still using the direct runtime seam.
...
The generic HTTP runtime seam when you want to point asta at any compatible orchestrator boundary.
...
That means `local`, `asta`, and `custom` are first-class saved profiles today.
...
The current provider review path should borrow from existing Hermes/Asta wizard flows where possible, but land as asta-native adapters instead of hard-coupling asta UI state to another project's connector code.
