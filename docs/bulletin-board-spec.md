...
> First concrete office-system feature for asta.
...
This should be the first step toward making asta a real agent operations environment instead of only a gateway visualizer.
...
The feature should hook into systems asta already has.
...
The board must not depend on Asta-specific methods.
...
- the system works with Hermes, Asta, and demo mode
...
The bulletin board should become the first shared memory surface inside asta.
