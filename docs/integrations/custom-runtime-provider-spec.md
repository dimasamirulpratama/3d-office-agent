...
> Generic extension seam for non-Asta, non-Hermes stacks.

## Goal

Define a clean `custom` runtime provider class for asta.

This provider should let external orchestration stacks integrate with asta through a stable seam without requiring:
...
- asta probes `GET /health`, `GET /state`, and `GET /registry`
- chat uses a direct HTTP path to `POST /v1/chat/completions`
- browser traffic is proxied through asta's same-origin
...
Not every useful runtime should have to become a named built-in provider in upstream asta.
...
asta’s runtime abstraction already points toward multiple providers:
...
It should not require upstream asta to know:
...
This gives asta enough identity for UI without hardcoding a brand into the provider class.
...
```ts
type RuntimeProviderId = "Asta" | "hermes" | "custom";
...
This keeps asta stable even if the custom stack has richer private event semantics internally.
...
The custom provider is the right place for richer internal state to enter asta.
...
The custom provider should be able to support office systems without asta needing to know the backend’s internals.
...
- make no runtime-specific assumptions in core asta

Actual runtime adapters can be implemented separately against that seam.

## Out of Scope For V1

- embedding proprietary internal orchestration logic in asta core
...
- upstream asta gains a clean extension seam
...
It gives asta extensibility, gives Vera a clean path in, and avoids hardwiring a personal stack directly into the core product model.
