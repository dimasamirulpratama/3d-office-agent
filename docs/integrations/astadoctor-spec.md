...
> First-pass diagnostics plan for asta deployments so users stop chasing the same setup failures manually.

## Goal

Provide a single diagnostics surface for the common "asta cannot connect"
...
but focused on asta's integration points across providers.
...
- does the endpoint respond like a asta-compatible gateway?
...
`astaoctor` should be designed against the runtime-profile model:
...
`astaoctor` should land before more runtime complexity because it will
make debugging:
...
`astaoctor` v1 should be considered complete when it provides:
...
- provider-aware checks for Asta, Hermes, demo, and custom runtimes
...
- deeper Asta pairing and device-approval detection
...
`astaoctor` v2 should also benefit from the separate runtime-profile work:
