...
> Reference implementation guide for plugging a non-Asta, non-Hermes runtime into asta through the upstream-safe `custom` provider seam.

## Goal

Show how a custom orchestration stack should plug into asta without requiring:
...
A custom runtime should sit at the asta boundary as an orchestrator-backed service.

That means:

- asta should talk to one stable runtime boundary
...
The `custom` provider exists so those stacks can integrate cleanly without forcing asta core to absorb stack-specific assumptions.

## Recommended Boundary

asta should integrate with the custom runtime's orchestrator or gateway layer, not with its individual workers.
...
This gives the UI enough identity without forcing upstream asta to branch on every runtime brand.
...
Do not claim support just because the backend can theoretically do something. Claim support only where the provider exposes a stable asta-facing behavior.

## Agent Mapping

asta agents should not be modeled as one fixed backend process each unless the runtime truly behaves that way.
...
asta sessions should map to runtime conversations or execution threads, not to whichever backend storage structure happens to exist internally.
...
The provider should translate runtime activity into the same normalized asta runtime events used elsewhere.
...
The custom provider is the right place to map richer internal runtime signals into asta's public office state model.
...
asta only needs the resulting office-safe state and maybe a public reason label.

## Relationship To Office Systems

The custom provider should support office systems without asta needing to know the backend's internals.
...
3. Map runtime roles or routes into asta agent identities.
...
This is enough to make a custom runtime useful in asta without overexposing internals.
...
- runtime-specific branches throughout asta core
- Asta compatibility shims
