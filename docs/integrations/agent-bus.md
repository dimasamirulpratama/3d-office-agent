...
> Visualize AI coding sessions in asta's retro office — without Asta.

[Agent Bus](https://github.com/emiliovos/agent-bus) is an open-source event routing system that bridges AI coding agents (Claude Code, Gemini, Codex, etc.) to asta. Agents appear in the 3D office, animate when working, and go idle between tasks. Zero inference cost — pure data routing.

## How It Works

```
AI coding session → hook fires → Agent Bus hub → gateway :18789 → asta renders in 3D
```

Agent Bus includes an **Asta-compatible gateway** that speaks the same WebSocket protocol asta already uses. No asta code changes needed — just point `GATEWAY_URL` to the Agent Bus gateway.
...
- Gateway on `:18789` (Asta protocol)

### Connect asta

Point asta's gateway URL to Agent Bus:

```bash
# In your asta .env or environment:
GATEWAY_URL=ws://localhost:18789
```

Restart asta. It will connect to the Agent Bus gateway instead of Asta.
...
Every tool use in Claude Code now appears as agent activity in asta.

## Gateway Protocol Compatibility

The Agent Bus gateway implements Asta protocol v2:
...
This exposes the hub and asta via HTTPS with service token authentication. Agents on remote machines (VPS, other PCs) can send events through the tunnel.
...
## Key Differences from Asta

| Feature | Asta | Agent Bus |
|---------|----------|-----------|
...
| Agent sources | Asta agents only | Any (Claude Code, Gemini, cron, etc.) |
