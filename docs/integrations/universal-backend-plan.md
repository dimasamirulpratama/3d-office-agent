...
> Backend-neutral asta integration plan for Asta, Hermes, Vera, and other runtimes.

## Recommendation

Do not treat PR #70 as the long-term integration architecture.

It is useful as a short-term compatibility shim and a source of a few good UX changes, but it does not make asta backend-neutral. It keeps asta Asta-shaped and makes Hermes imitate Asta.
...
- Every future backend would otherwise need to keep emulating the Asta gateway protocol.

The better path is:

1. Keep Asta support intact.
2. Extract a backend-neutral runtime adapter inside asta.
...
- A full Asta-protocol emulator as the primary Hermes integration.
...
asta should stop treating the browser gateway client as the backend abstraction.
...
The browser can still use WebSocket streaming from Studio, but the messages should be asta-native runtime events rather than implicitly Asta events.
...
If a provider does not support a surface, asta should disable or hide the UI for it. It should not fake a successful write.
...
3. Asta-protocol shim only as a temporary bridge.
...
- The Asta shim should be treated as transitional compatibility, not the permanent contract.
...
The Vera provider should map asta agent identities to routed roles or lanes rather than pretending Vera is an Asta gateway.

## Event Model

Current asta expects Asta-flavored `chat`, `agent`, and `presence` events.

That is too narrow for universal providers. Studio should normalize provider-native updates into a asta event model with explicit semantics:
...
- Wrap current Asta behavior in an `Asta` provider.
...
- Map asta agents to Vera roles or lanes.
...
This path avoids making Vera imitate Asta.

Instead, Vera can appear as:

- a routed multi-role intelligence backend,
- with asta visualizing agents, runs, status, and streamed text,
- while preserving Vera-specific routing, lane, and model identity.

That gives asta a broader identity:

- similar to the Asta ecosystem,
- but not subordinate to Asta's protocol and assumptions.
...
- wrap existing Asta integration behind it,
