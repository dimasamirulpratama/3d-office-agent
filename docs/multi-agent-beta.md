...
This document explains the current multi-agent beta in asta: what it does, how the two connection modes work, and how to connect a second office.

## What This Beta Does

asta can render a second office inside the same 3D scene so you can visualize agents from another machine.
...
- **Local office**: the asta instance you are currently using;
- **Remote office**: another asta instance or another Asta gateway you want to visualize.

The remote office can be connected in one of two ways:

1. **Remote asta presence endpoint**.
2. **Remote Asta gateway**.

## Connection Modes

### 1. Remote asta Presence Endpoint

Use this when the other machine is also running asta.

How it works:

- your local asta server polls the remote asta `presence` endpoint;
...
This mode is best when you want the remote side to feel like another full asta office.

### 2. Remote Asta Gateway

Use this when the other machine only runs Asta and does not run asta.

How it works:

- the browser connects directly to the remote gateway;
- asta derives a read-only presence snapshot from gateway data such as `agents.list`, `status`, and `sessions.preview`;
- because there is no remote asta layout endpoint, the second office uses a fallback office visualization.
...
If you paste an `http://` or `https://` URL into gateway mode, asta normalizes it to `ws://` or `wss://` before connecting.

This mode is best when you want remote agent visibility without requiring a second asta deployment.
...
- is designed to avoid exposing remote files or tool output in the asta UI.
...
Under the hood, asta now uses a shared runtime contract for:

- `agents.message`
- `agents.handoff`

Asta, Hermes, Demo, and direct custom/local/asta runtime profiles can all target the same message/handoff seam. Provider-native adapters such as Anthropic or Claude Code are still a follow-up slice.
...
- your local asta is already working with your local Asta gateway;
...
1. Start your local asta instance.
...
### Setup For `Remote asta presence endpoint`

Use:

- `Source type`: `Remote asta presence endpoint`.
- `Presence URL`: the remote `/api/office/presence` URL.
- `Optional token`: only if that remote asta endpoint is protected.
...
- if the remote layout snapshot is unavailable, asta falls back to a default/fallback office rendering for the remote side.

### Setup For `Remote Asta gateway`

Use:

- `Source type`: `Remote Asta gateway`.
...
- the office shell is a fallback visualization, not a true remote layout clone from another asta instance.

## Recommended Network Patterns

### Same private network

Use a reachable private IP or local hostname for the remote asta endpoint or Asta gateway.
...
- remote asta endpoint over `https://<machine>.ts.net/api/office/presence`;
- remote Asta gateway over `wss://<machine>.ts.net` if you are proxying the gateway through HTTPS/WSS;
...
That usually means the other machine has asta presence available but not a layout snapshot yet. The beta should still render a fallback remote office.
...
Opening a web page in the browser does not automatically mean the Asta gateway WebSocket is reachable.
...
- Gateway mode derives presence from gateway snapshots rather than a real remote asta layout.
...
Use `Remote asta presence endpoint` when the other side runs asta and you want the most complete office visualization.

Use `Remote Asta gateway` when the other side only runs Asta and you mainly want remote agent presence plus lightweight text messaging.
