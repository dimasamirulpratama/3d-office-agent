...
- How “create agent” choices in **asta** flow into the **Asta Gateway** (often running on an EC2 host) where enforcement actually happens

Scope:
- Studio one-step agent creation and post-create authority updates, including exact gateway calls.
- The upstream Asta implementation that persists and enforces those settings at runtime.
...
1. Writes **configuration** into the Gateway (per-agent overrides in `Asta.json`).
2. Writes **policy** into the Gateway (per-agent exec approvals in `exec-approvals.json`).

The Gateway (Asta) is the enforcement point:
...
- **Gateway**: Asta Gateway WebSocket server (upstream project).
- **Studio**: this repo. Next.js UI plus a Node WS proxy.
- **Agent**: an Asta agent entry stored in gateway config (`agents.list[]`).
- **Session key**: Asta session identifier. Studio uses `agent:<agentId>:<mainKey>` for the agent’s “main” session.
...
- **Tool policy** (`tools.profile`, `tools.alsoAllow`, `tools.deny`): allow/deny gating for PI tools (Asta resolves effective policy).
...
Studio capability updates rely on Asta tool-group expansion (`Asta/src/agents/tool-policy.ts`), especially:
...
## Asta (Upstream): What `agents.create` Actually Does

Gateway method:
- `Asta/src/gateway/server-methods/agents.ts` (`"agents.create"`)
...
## Asta (Upstream): Sandbox Semantics

Sandbox configuration resolution:
- `Asta/src/agents/sandbox/config.ts` (`resolveSandboxConfigForAgent`)

Sandbox context creation (where workspace selection happens):
- `Asta/src/agents/sandbox/context.ts` (`resolveSandboxContext`)

Docker mount behavior:
- `Asta/src/agents/sandbox/docker.ts` (`createSandboxContainer`)
...
- `Asta/src/agents/sandbox/runtime-status.ts` (`resolveSandboxRuntimeStatus`)
...
Upstream implementation reference:
- `Asta/src/agents/sandbox/types.ts` (`SandboxScope`)
- `Asta/src/agents/sandbox/shared.ts` (`resolveSandboxScopeKey`)
...
- `Asta/src/agents/sandbox/constants.ts` uses `<STATE_DIR>/sandboxes` (where `STATE_DIR` defaults to `~/.Asta` unless overridden by `Asta_STATE_DIR`).
...
- When using a sandbox workspace root, upstream seeds missing bootstrap files from the agent workspace and ensures bootstrap exists:
  - `Asta/src/agents/sandbox/workspace.ts` (`ensureSandboxWorkspace`)
...
In upstream Asta, sandboxed filesystem tools are rooted and guarded:
- `Asta/src/agents/pi-tools.read.ts` (`assertSandboxPath` usage)
...
Asta has an additional sandbox-only tool allow/deny policy:
- `tools.sandbox.tools.allow|deny` (global)
- `agents.list[].tools.sandbox.tools.allow|deny` (per-agent override)

Upstream resolution:
- `Asta/src/agents/sandbox/tool-policy.ts` (`resolveSandboxToolPolicyForAgent`)
...
## Asta (Upstream): Tool Availability and `workspaceAccess=ro`

PI tool construction:
- `Asta/src/agents/pi-tools.ts` (`createAstaCodingTools`)
...
Separately from per-agent config and exec approvals, Asta supports per-session exec settings:
...
- Upstream method: `Asta/src/gateway/server-methods/sessions.ts` (`"sessions.patch"`)
- Patch application: `Asta/src/gateway/sessions-patch.ts`
- Session entry shape includes `execHost|execSecurity|execAsk`: `Asta/src/config/sessions/types.ts`
...
## Asta (Upstream): Exec Approvals (Policy + Events)

Exec approvals file (defaults upstream):
- `Asta/src/infra/exec-approvals.ts`
  - default file path: `~/.Asta/exec-approvals.json`
  - default socket path: `~/.Asta/exec-approvals.sock`

Gateway methods (persist policy):
- `Asta/src/gateway/server-methods/exec-approvals.ts`
...
- `Asta/src/gateway/server-methods/exec-approval.ts`
  - broadcasts `exec.approval.requested`
  - broadcasts `exec.approval.resolved`

Exec tool approval decision logic:
- `Asta/src/agents/bash-tools.exec.ts` (calls `requiresExecApproval`, `evaluateShellAllowlist`, etc.)
...
1. Determine if the session is sandboxed and what workspace it is using.
   - Upstream CLI helper: `Asta sandbox explain --agent <agentId>` (see upstream `src/commands/sandbox-explain.ts`)
