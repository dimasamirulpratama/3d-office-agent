...
This file captures the near-term direction for asta so outside contributors can find work that aligns with current priorities.
...
## Product Ideas To Reduce Asta Dependency
...
- Add config diff and rollback tools so gateway-wide changes can be reviewed and safely reverted from asta.
