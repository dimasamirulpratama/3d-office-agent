...
asta can run against Hermes by using the bundled adapter in
...
asta-compatible WebSocket adapter.
...
The frontend keeps using the asta gateway protocol. The Hermes adapter
...
In the connect screen, select `Hermes backend`. asta will persist that
...
The adapter currently supports the asta surfaces needed for normal
office use:
...
- `scripts/astad3d-start.sh` no longer hardcodes one developer's local path
...
asta gateway contract today and is ready for upstream testing now.
...
That starts a bundled mock gateway for a no-framework asta demo.

## Using Asta instead

If you want the Asta path, do not run the Hermes adapter. Start
Asta and point asta at that gateway instead.
