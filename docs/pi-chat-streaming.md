...
This document exists to onboard coding agents quickly when debugging chat issues in asta.

Scope:
- Describes how Studio connects to the Asta Gateway, how runtime streaming arrives over WebSockets, and how the UI renders it.
- Treats **PI** as “the coding agent running behind the Gateway” (an Asta agent). Studio does not implement PI logic; it displays and controls the Gateway session.

Non-scope:
- PI internals and model/tool execution details. Those live in the Asta repository and the Gateway implementation.
...
- Browser gateway protocol client (vendored): `src/lib/gateway/Asta/GatewayBrowserClient.ts`
...
## Relationship To Asta (What’s Vendored Here)

Studio vendors the browser Gateway client used to speak the Gateway protocol:
- Vendored client: `src/lib/gateway/Asta/GatewayBrowserClient.ts`
- Sync script: `scripts/sync-Asta-gateway-client.ts`
...
## Upstream Source Of Truth (Asta)

For chat streaming behavior, these upstream files are authoritative:
- `src/gateway/protocol/schema/logs-chat.ts` in your Asta checkout (`chat.send`, `chat.history`, and chat event schema)
- `src/gateway/server-methods/chat.ts` in your Asta checkout (`chat.send` ack + idempotency, `chat.history` payload shaping/sanitization)
- `src/gateway/server-chat.ts` in your Asta checkout (`agent` event fanout and synthetic `chat` delta/final bridging)
- `src/agents/pi-embedded-subscribe.ts` and related handlers in your Asta checkout (`assistant`/`tool`/`lifecycle` stream emission)
...
- Gateway (upstream): the Asta Gateway WebSocket server (default `ws://localhost:18789`).
...
sequenceDiagram
  ...
  participant G as Asta Gateway (upstream)
...
- `~/.Asta/asta/settings.json` (see `README.md` for the canonical location)
...
- The Studio server resolves an upstream token from `asta/settings.json`, and if it is missing it may fall back to the local Asta config in `Asta.json` (token + port). This behavior exists in both the WS proxy path (`server/studio-settings.js`) and the `/api/studio` storage layer (`src/lib/studio/settings-store.ts`) and they should remain consistent.
...
- Transport reconnect (after a successful hello): the vendored browser client reconnects the browser->Studio WebSocket with backoff when it closes, and continues emitting events after reconnect. See `src/lib/gateway/Asta/GatewayBrowserClient.ts`.
...
Files:
- Gap detection: `src/lib/gateway/Asta/GatewayBrowserClient.ts`
...
- `MEDIA: /home/ubuntu/.Asta/.../image.png`
...
3. The API route reads the image either locally (only under `~/.Asta`) or over SSH for remote gateways, and returns the bytes with the correct `Content-Type`.

Files:
- Rewrite helper: `src/lib/text/media-markdown.ts`
- Media API route: `src/app/api/gateway/media/route.ts`
- SSH helper + env vars (`Asta_GATEWAY_SSH_TARGET`, `Asta_GATEWAY_SSH_USER`): `src/lib/ssh/gateway-host.ts`
