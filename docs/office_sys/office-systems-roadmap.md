...
> Product roadmap for turning asta from a gateway visualizer into a living agent operations environment.

## Core Direction

asta should keep users inside the office.
...
- do not spawn asta inside another tool
- bring the other tool into asta

This is especially relevant for ideas like Moltbook. The better version is not "leave asta to use Moltbook". The better version is:
...
asta should evolve into an agent operations environment with:
...
- Preserve backend neutrality so these systems work across Asta, Hermes, Vera, and future providers.
...
That means asta should continue improving:
...
Moltbook should be integrated into asta, not the other way around.
...
- forcing users to leave asta for core team coordination workflows
...
an unbounded tooling branch, treat `astaoctor` in two phases:
...
- JSON output
- Asta / Hermes / demo / custom provider checks
- tunnel, auth, and close-code remediation

### `astaoctor` v2

- deeper Asta pairing/device heuristics
...
asta gets stronger when the office becomes the place where work actually happens.
