...
This document explains skills from first principles, how they work in the Asta runtime, and how asta currently exposes them in UX.

It is intended as design context for rethinking the Skills UX.

## 1) Why skills exist (first principles)

Skills are the mechanism Asta uses to give agents reusable operational know-how without hardcoding that know-how into core runtime logic.
...
Asta intentionally uses AgentSkills-compatible `SKILL.md` structure and semantics.

Why this matters:
- Interoperability: skills can move between ecosystems that understand AgentSkills.
- Community/network effects: external skill ecosystems (for Asta specifically, AstaHub) can be leveraged instead of reinventing proprietary formats.
- UX consistency: users can reason about “a skill folder with `SKILL.md` + metadata gates” instead of app-specific abstractions.

Asta adds product-specific metadata under `metadata.Asta` (install specs, gating fields, primary env key, etc.) while keeping the base skill shape compatible.
...
Important optional fields used by Asta:
- `metadata.Asta.always`
- `metadata.Asta.skillKey`
- `metadata.Asta.primaryEnv`
- `metadata.Asta.os`
- `metadata.Asta.requires.{bins, anyBins, env, config}`
- `metadata.Asta.install[]`
- `user-invocable`
...
- Parsed frontmatter
- Resolved Asta metadata
- Invocation policy flags

## 4) Where skills come from (discovery + precedence)

Asta merges multiple sources into one effective skill set.

Current merge precedence in code (lowest -> highest):
1. `skills.load.extraDirs` and plugin-contributed skill dirs (`source: Asta-extra`)
2. Bundled skills (`Asta-bundled`)
3. Managed/global local skills (`~/.Asta/skills`, `Asta-managed`)
...
6. Workspace skills (`<workspace>/skills`, `Asta-workspace`)
...
Asta has a separate per-agent skill filter via `agents.list[].skills`:
...
Sandbox nuance:
- For non-`rw` sandbox workspaces, Asta syncs skills into sandbox workspace (best-effort) so skill files remain accessible.
...
- `skills.update` writes gateway config (`Asta.json`) and is gateway-wide state mutation.

Security detail:
- `skills.status` exposes config check satisfaction, not raw secret config values.

## 10) asta UX (current behavior)
...
- Only specific sources removable (`Asta-managed`, `Asta-workspace`).
...
Sorted by most recent creation time in `asta/.agent/done`, the latest items are mostly bugfix exec plans (streaming, proxy auth, stale config, cron rollback, etc.).
