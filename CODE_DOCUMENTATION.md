...
This file is the practical code map for asta contributors.
...
asta is the UI and local Studio/proxy layer around an existing Asta Gateway.

- Asta owns agent execution, sessions, tools, config, and runtime events.
- asta owns visualization, local Studio settings, UI workflows, office rendering, and the same-origin WebSocket/API bridge.
...
- `server/gateway-proxy.js` bridges browser WebSocket traffic to the upstream Asta Gateway.
...
- `scripts/sync-Asta-gateway-client.ts` updates the vendored gateway client helpers.
...
2. Studio proxies that connection to the upstream Asta Gateway.
...
- This repo is not the Asta runtime. Do not modify upstream Asta source code from here.

## When You Need Upstream Asta Context

Sometimes asta behavior depends on the upstream event contract or session behavior. In those cases:

1. Inspect the relevant client or gateway contract in `src/lib/gateway`.
2. If the answer is not in this repo, inspect your separate local Asta checkout.
3. Apply changes in asta unless the user explicitly asked for upstream Asta work.
